...
CREATE TABLE `logs` (`id` BIGINT,`user_id` BIGINT,`created_at` BIGINT,`type` BIGINT,`content` LONGTEXT,`username` VARCHAR(255) DEFAULT '',`token_name` VARCHAR(255) DEFAULT '',`model_name` VARCHAR(255) DEFAULT '',`quota` BIGINT DEFAULT 0,`prompt_tokens` BIGINT DEFAULT 0,`completion_tokens` BIGINT DEFAULT 0,`use_time` BIGINT DEFAULT 0,`is_stream` TINYINT(1),`channel_id` BIGINT,`channel_name` VARCHAR(255),`token_id` BIGINT DEFAULT 0,`group` VARCHAR(255),`ip` VARCHAR(255) DEFAULT '',`other` LONGTEXT,PRIMARY KEY (`id`));
...
CREATE TABLE `tokens` (`id` BIGINT,`user_id` BIGINT,`key` char(48),`status` BIGINT DEFAULT 1,`name` VARCHAR(255),`created_time` BIGINT,`accessed_time` BIGINT,`expired_time` BIGINT DEFAULT -1,`remain_quota` BIGINT DEFAULT 0,`unlimited_quota` TINYINT(1),`model_limits_enabled` TINYINT(1),`model_limits` varchar(1024) DEFAULT "",`allow_ips` VARCHAR(1000) DEFAULT "",`used_quota` BIGINT DEFAULT 0,`group` VARCHAR(255) DEFAULT "",`deleted_at` DATETIME,PRIMARY KEY (`id`));
...
CREATE TABLE `quota_data` (`id` BIGINT,`user_id` BIGINT,`username` VARCHAR(255) DEFAULT "",`model_name` VARCHAR(255) DEFAULT "",`created_at` BIGINT,`token_used` BIGINT DEFAULT 0,`count` BIGINT DEFAULT 0,`quota` BIGINT DEFAULT 0,PRIMARY KEY (`id`));
CREATE TABLE `channels` (`id` BIGINT,`type` BIGINT DEFAULT 0,`key` LONGTEXT NOT NULL,`open_ai_organization` LONGTEXT,`test_model` LONGTEXT,`status` BIGINT DEFAULT 1,`name` VARCHAR(255),`weight` BIGINT DEFAULT 0,`created_time` BIGINT,`test_time` BIGINT,`response_time` BIGINT,`base_url` LONGTEXT DEFAULT "",`other` LONGTEXT,`balance` DOUBLE,`balance_updated_time` BIGINT,`models` LONGTEXT,`group` varchar(64) DEFAULT "default",`used_quota` BIGINT DEFAULT 0,`model_mapping` LONGTEXT,`status_code_mapping` varchar(1024) DEFAULT "",`priority` BIGINT DEFAULT 0,`auto_ban` BIGINT DEFAULT 1,`other_info` LONGTEXT,`tag` VARCHAR(255),`setting` LONGTEXT,`param_override` LONGTEXT,`channel_info` json,PRIMARY KEY (`id`));
INSERT INTO channels VALUES(1,0,'secret','','gpt-4.1',1,'PollinationsAI',0,1755756103,NULL,NULL,'http://68.183.101.45:8080','{"endpoint_type": "openai", "support_models": true}',NULL,NULL,'gpt-4.1','default',0,'null','{"400": "500"}',0,1,NULL,NULL,'{"force_format": true}','{"temperature": 0}',X'7b2269735f6d756c74695f6b6579223a66616c73652c226d756c74695f6b65795f73697a65223a302c226d756c74695f6b65795f7374617475735f6c697374223a6e756c6c2c226d756c74695f6b65795f706f6c6c696e675f696e646578223a302c226d756c74695f6b65795f6d6f6465223a2272616e646f6d227d');
